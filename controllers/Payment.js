...
exports.verifySignature = async (req, res) => {
    ...
    // validation 
    if(!razorpayOrderId || !razorpayPaymentId || !razorpaySignature || !courses || !userId) return res.status(400).json({ message: "Payment details missing" });
...
const enrollStudent = async (courses, userId ,res) => {
  try {
    if(!courses || !userId) return res.status(400).json({ message: "Please provide courses and userId" });
    ...
    for(const courseId of courses){
        ...
        const emailResponse = await mailSender(
            ...
            courseEnrollmentEmail( enrolledCourse.name ,`${enrolledStudent.firstName } ${enrollStudent.lastName}` )
        )
    }
...
exports.sendPaymentSuccessEmail = async (req, res) => {
    ...
    const userId = req.user.id;
    // validation
    if(!orderId || !paymentId || !amount || !userId) return res.status(400).json({success:false, message: "Payment details missing" });

    try {
        // find the student
        const enrolledStudent = await User.findOne({ _id: userId });
        await mailSender(enrolledStudent.email, "Payment Successful", paymentSuccess(amount/100, paymentId, orderId, enrolledStudent.firstName, enrolledStudent.lastName))
